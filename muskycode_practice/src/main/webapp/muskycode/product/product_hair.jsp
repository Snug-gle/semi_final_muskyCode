...
<%
	...
	int totalProduct = ProductDAO.getDAO().selectStatusProductCount("HAIR");
	...
	List<ProductDTO> productList = ProductDAO.getDAO().selectProductList(startRow, endRow, "HAIR");
...
<hr class="layout"/>
	...
	<div class="titleArea" >
		<div id="product_title"><h2>HAIR</h2>
	</div>
	<br><hr><br>
...
<br><br>
	...
	<% if(startPage>blockSize) { //시작페이지 번호가 5페이지(blockSize)를 초과할 경우 링크 설정 %>
		<a href="<%=request.getContextPath() %>/muskycode/index.jsp?muskygroup=product&musky=product_hair&pageNum=1" class="first"><img src="<%=request.getContextPath() %>/muskycode/images/btn_page_first.gif" alt="첫 페이지"/></a>
		<a href="<%=request.getContextPath() %>/muskycode/index.jsp?muskygroup=product&musky=product_hair&pageNum=<%=startPage-blockSize %>"><img src="<%=request.getContextPath() %>/muskycode/images/btn_page_prev.gif" alt="이전 페이지"/></a>
	...
	<% for(int i=startPage; i<=endPage; i++) { %>
		<% if(pageNum!=i) { //요청페이지 번호와 출력페이지 번호가 같지 않은 경우 링크 설정 %>
			<li><a style="position:relative; display: block;" class="other" href="<%=request.getContextPath() %>/muskycode/index.jsp?muskygroup=product&musky=product_hair&pageNum=<%=i %>">&nbsp;<%=i %>&nbsp;</a></li>
		<% } else { //요청페이지 번호와 출력페이지 번호가 같을 경우 링크 미설정 %>
			<li class="xans-record-"><%=i %></li>
		<% } %>
	<% } %>
	</ol>
	
	<% if(endPage!=totalPage) { //종료페이지 번호와 전체페이지 개수가 같지 않은 경우 링크 설정 %>
		<a href="<%=request.getContextPath() %>/muskycode/index.jsp?muskygroup=product&musky=product_hair&pageNum=<%=startPage+blockSize %>"><img src="<%=request.getContextPath() %>/muskycode/images/btn_page_next.gif" alt="다음 페이지"/></a>
		<a href="<%=request.getContextPath() %>/muskycode/index.jsp?muskygroup=product&musky=product_hair&pageNum=<%=totalPage %>" class="last"><img src="<%=request.getContextPath() %>/muskycode/images/btn_page_last.gif" alt="마지막 페이지"/></a>

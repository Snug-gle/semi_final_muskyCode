...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- 글번호를 전달받아 QNA 테이블에 저장된 해당 글번호의 게시글을 검색하여 클라이언트에게 전달하는 JSP 문서 --%>
...
<%
	...
	//글번호를 전달받아 QNA 테이블에 저장된 해당 글번호의 게시글에 조회수를 증가하는 DAO 클래스의 메소드 호출
...
<div id="container" style="position: relative; bottom:100px; width: 1000px;">
	...
	<div id="qna_menu">
		...
		<% } %>
<%-- 		<% if(loginMember!=null && (loginMember.getMID().equals(qna.getMID())  --%>
<%-- 				||loginMember.getMGRADE()==9)) {//사용자가 글쓴이 이거나 관리자일 경우 답글쓰기 활성화 %> --%>
...
<%-- 		<% } %> --%>
	...
	<form id="boardForm" method="post">
		...
		<input type="hidden" name="ref" value="<%=qna.getQREF()%>"> <input
			type="hidden" name="reStep" value="<%=qna.getQREF_STEP()%>">
...
$("#removeBtn").click(function() {
	if(confirm("게시글을 삭제 하시겠습니까?")) {
		$("#boardForm").attr("action", "<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=board&musky=qna_remove_action");
		$("#boardForm").submit();
	}
});

$("#modifyBtn").click(function() {
	$("#boardForm").attr("action", "<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=board&musky=qna_modify");
	$("#boardForm").submit();
});

$("#replyBtn").click(function() {
	$("#boardForm").attr("action", "<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=board&musky=qna_write");
	$("#boardForm").submit();
});

$("#listBtn").click(function() {
	$("#boardForm").attr("action", "<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=board&musky=qna");
	$("#boardForm").submit();
});
</script>

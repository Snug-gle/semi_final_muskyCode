...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- 카테고리를 전달받아 NOTICE 테이블에 저장된 해당 카테고리의 공지글 정보를 검색하여 클라이언트에게 전달하는 JSP 문서 --%>
...
<%
	...
	//카테고리를 전달받아 NOTICE 테이블에 저장된 해당 카테고리의 제품정보를 검색하여 
...
<hr class="layout"/>
	...
	<div id="notice_title"  class="titleArea" >
		<h2 style="color: blue;">NOTICE MANAGE [총 <%=totalNotice %>건]</h2>
	</div>

<div class="ec-base-table typeList gBorder">

	<% if (loginMember != null && loginMember.getMGRADE() == 9) {//로그인 사용자와 관리자인 경우 %>
	<div id="btn">
		<button type="button" class="btn_black"  style="position: relative; left: 430px; bottom:10px;"
			onclick="location.href='<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=notice_write';">글쓰기</button>
	</div>
	<%	} %>
			...
			<tbody class="xans-element- xans-board xans-board-list-1002 xans-board-list xans-board-1002 center">
				...
				<tr>
					...
					<td class="subject" style="text-align: left; text-indent: 20px;">
		 			<a href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=notice_detail&num=<%=notice.getNNO()%>&pageNum=<%=pageNum%>&search=<%=search%>&keyword=<%=keyword%>"><%=notice.getNTITLE()%></a>
					</td>
					<%-- 조회수 --%>
	...
	<a href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=notice_manage&pageNum=1&search=<%=search%>&keyword=<%=keyword%>">[처음]</a>
	<a href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=notice_manage&pageNum=<%=startPage - blockSize%>&search=<%=search%>&keyword=<%=keyword%>">[이전]</a>
	...
	<a href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=notice_manage&pageNum=<%=i%>&search=<%=search%>&keyword=<%=keyword%>">[<%=i%>]
	</a>
	...
	<a href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=notice_manage&pageNum=<%=startPage + blockSize%>&search=<%=search%>&keyword=<%=keyword%>">[다음]</a>
	<a href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=notice_manage&pageNum=<%=totalPage%>&search=<%=search%>&keyword=<%=keyword%>">[마지막]</a>
	<% } else {//종료 페이지 번호와 전체 페이지 갯수가 같은 경우 링크 미설정 %>
	[다음][마지막]
	<% } %>
	<br><br>
	<%-- 게시글 검색 기능 제공 --%>
	<form action="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=notice_manage" method="post">

...
<div id = "review_title"  class="titleArea">
		<h2 style="color: blue;">REVIEW MANAGE</h2>
</div>
	...
	<% } else { %>
		...
		<tr>
			...
			<% if(review.getRSTATUS()==0) {//삭제 게시글인 경우 %> <span
				...
				<% }else {%>
			<a href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=review_detail&num=<%=review.getRNO()%>&pageNum=<%=pageNum%>&search=<%=search%>&keyword=<%=keyword%>"><%=review.getRTITLE()%></a>
				<%} %>
			</td>
...
<% if(startPage>blockSize) {//시작 페이지 번호가 blockSize 변수값을 초과한 경우 링크 설정 %>
	<a
		href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=review_manage&pageNum=1&search=<%=search%>&keyword=<%=keyword%>">[처음]</a>
	<a
		href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=review_manage&pageNum=<%=startPage-blockSize%>&search=<%=search%>&keyword=<%=keyword%>">[이전]</a>
	...
	<a
		href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=review_manage&pageNum=<%=i%>&search=<%=search%>&keyword=<%=keyword%>">[<%=i%>]
	</a>
	...
	<a
		href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=review_manage&pageNum=<%=startPage+blockSize%>&search=<%=search%>&keyword=<%=keyword%>">[다음]</a>
	<a
		href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=review_manage&pageNum=<%=totalPage%>&search=<%=search%>&keyword=<%=keyword%>">[마지막]</a>
	...
	<%-- 게시글 검색 기능 제공 --%>
	<form
		action="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=review_manage"
		method="post">

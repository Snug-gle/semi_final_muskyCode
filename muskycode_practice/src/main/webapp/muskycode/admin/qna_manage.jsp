...
//검색 관련 전달값을 반환받아 저장
	...
	//JSP 문서를 요청하여 전달된 페이지 번호를 반환받아 저장
		...
		//요청 페이지에 대한 게시글 시작 행번호와 게시글 종료 행번호를 전달받아 QnA 테이블에 저장된
...
<div id="board_list" class="titleArea" >
		<div id="board_title"><h2 style="color: blue;">Q&A MANAGE</h2></div>
</div>
<div class="ec-base-table typeList gBorder">
			...
			<tbody class="xans-element- xans-board xans-board-list-1002 xans-board-list xans-board-1002 center">
				...
				<tr>
					...
					<td class="qtitle" style="text-align: left; text-indent: 20px;">
						...
						<a href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=qna_detail&num=<%=qna.getQNO() %>&pageNum=<%=pageNum %>&search=<%=search%>&keyword=<%=keyword%>"><%=qna.getQTITLE() %></a>
					...
					<% if(qna.getQSTATUS()!=0) {//삭제 게시글이 아닌 경우 %>
						...
						<td>
							...
							<% if(currentDate.equals(qna.getQDATE().substring(0, 10))) { %>
	...
	<a
		href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=qna_manage&pageNum=1&search=<%=search%>&keyword=<%=keyword%>">[처음]</a>
	<a
		href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=qna_manage&pageNum=<%=startPage-blockSize%>&search=<%=search%>&keyword=<%=keyword%>">[이전]</a>
	...
	<a
		href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=qna_manage&pageNum=<%=i%>&search=<%=search%>&keyword=<%=keyword%>">[<%=i%>]
	</a>
	...
	<a
		href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=qna_manage&pageNum=<%=startPage+blockSize%>&search=<%=search%>&keyword=<%=keyword%>">[다음]</a>
	<a
		href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=qna_manage&pageNum=<%=totalPage%>&search=<%=search%>&keyword=<%=keyword%>">[마지막]</a>
	...
	<%-- 게시글 검색 기능 제공 --%>
	<form
		action="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=admin&musky=qna_manage"
		method="post">

...
//QNA 테이블 (본인) 전체 게시물수 반환
int totalQna = QnaDAO.getDAO().selectIdQnaCount(loginMember.getMID());
...
//요청 페이지에 대한 게시글 시작 행번호와 게시글 종료 행번호와 회원 아이디를 전달받아 QNA 테이블에 저장된
//해당 행범위의 게시글을 검색하여 반환하는 DAO 클래스의 메소드 호출
List<QnaDTO> qnaList=QnaDAO.getDAO().SelectQnaIdList(startRow, endRow,loginMember.getMID());
...
<hr class="layout"/>
	...
	<div id="qna_title"  class="titleArea" >
		<h2>MY Q&A</h2>
	</div>
...
<div class="ec-base-table typeList gBorder">
	...
	<a
		href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=mypage&musky=my_qna&pageNum=1">[처음]</a>
	<a
		href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=mypage&musky=my_qna&pageNum=<%=startPage-blockSize%>">[이전]</a>
	...
	<a
		href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=mypage&musky=my_qna&pageNum=<%=i%>">[<%=i%>]
	</a>
	...
	<a
		href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=mypage&musky=my_qna&pageNum=<%=startPage+blockSize%>">[다음]</a>
	<a
		href="<%=request.getContextPath()%>/muskycode/index.jsp?muskygroup=mypage&musky=my_qna&pageNum=<%=totalPage%>%>">[마지막]</a>
